...
app.post("/urls", (req, res) => {
  ...
  const templateVars = {
    username: req.session.userId.email,
    shortURL: newID,
    longURL: req.body.longURL
  };
...
app.get("/urls/:shortURL", (req, res) => {
  ...
  const templateVars = { username: req.session.userId.email, shortURL, longURL };
  res.render("urls_show", templateVars);
});
...
app.post("/urls/:shortURL/edit", (req, res) => {
  ...
  const templateVars = { username: req.session.userId.email, urls: filteredDatabase(req.session.userId.id)};
  res.render("urls_index", templateVars);
});
...
app.get("/urls", (req, res) => {
  if (req.session.userId === undefined) {
    ...
    let templateVars = {
      username: req.session.userId.email,
      urls: filteredDatabase(req.session.userId.id)
    };
    res.render("urls_index", templateVars);
  }
});
